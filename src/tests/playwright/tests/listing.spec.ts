...
test('create listing', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Postani Prodavač' }).click();
  ...
  await page.getByRole('textbox', { name: 'Tip opreme:' }).fill('Skije');
  await page.getByRole('textbox', { name: 'Tip opreme:' }).press('Tab');
  await page.getByRole('textbox', { name: 'Stanje opreme:' }).fill('Odlično');
